...
test.describe('ConfluenceSearchModal', () => {
	...
	test('table and table text in display view dropdown render correctly when table view is selected', async ({
		...
		await expect(page.getByTestId('confluence-search-datasource-table')).toBeVisible();
	});
	...
	test('Inline smart card for inline view and Inline link text in dropdown render correctly when Inline link view is selected', async ({
		...
		await expect(page.getByRole('link', { name: 'https://hello.atlassian.net/' })).toBeVisible();
	});
	...
	test('should reset and clear hydrated filter values after a site change', async ({ page }) => {
		...
		await expect(page.getByText('Edited or created by: Atlassian Assist (staging)')).toBeVisible();
		...
		await expect(page.getByTestId('datasource-modal--initial-state-view')).toBeVisible();
	});
});
