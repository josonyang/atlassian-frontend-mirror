			...
			suggest: [
				{
					...
					fix(fixer) {
						...
						const attributesText = node.openingElement.attributes
							.filter((attr) => !isNodeOfType(attr, 'JSXAttribute') || attr.name.name !== 'role')
							...
							if (node.openingElement.selfClosing) {
								fixers.push(
									fixer.replaceTextRange(
										...
										`${imageName}${attributesText ? ` ${attributesText}` : ''} /`,
									),
								);
							} else {
								fixers.push(
						...
						if (closingTagRange && !node.openingElement.selfClosing) {
							fixers.push(
								fixer.replaceTextRange([closingTagRange[0] + 2, closingTagRange[1] - 1], imageName),
							);
						}
