...
class JSXExpressionLinter {
	...
	private analyzeIdentifier(sourceIdentifier: Scope.Reference['identifier']) {
		...
		const spreadProperties =
			...
			findSpreadProperties(identifier.parent.init.arguments[0] as ES.ObjectExpression);
		if (spreadProperties) {
		...
		if (this.configuration.stylesPlacement === 'bottom') {
			...
			fixerNodePlacement =
				(programNode.body.length === 1
					? programNode.body[0]
					: programNode.body.find((node: ES.Node) => node.type !== 'ImportDeclaration')) ??
				fixerNodePlacement;
		}
...
const rule: Rule.RuleModule = createLintRule({
	...
	create(context: Rule.RuleContext) {
		const mergedConfig: Required<RuleConfig> = assign({}, defaultConfig, context.options[0]);
