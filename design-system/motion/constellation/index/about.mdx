...
The recommended way to apply entry and exit animations is with the [Motion primitive](/components/motion/motion-primitive) component. Pass
pre-defined motion tokens to `enteringAnimation` and `exitingAnimation`, or supply custom `cssMap`
styles via `enteringAnimationXcss` and `exitingAnimationXcss` for full control.
...
For new CSS-based animations that are not entry or exit animations, use the `motion.duration.*` and `motion.easing.*` design tokens to define animation and transition CSS properties:
...
particular motion that doesn't exist yet, consider contributing motion design tokens to `@atlaskit/tokens` so every app
at Atlassian can benefit.
...
respect the user's reduced-motion preference. See
[Accessibility](/components/motion/accessibility) for the utilities available.
...
<SectionMessage appearance="warning" title="Caution">
  <p>
    FadeIn, SlideIn, ZoomIn and ShrinkOut are legacy components and are planned for deprecation.
    For new work, use the{' '}<a href="/components/motion/motion-primitive">Motion primitive</a> component
    instead.
  </p>
</SectionMessage>
...
[Entering motions](/components/motion/entering-motions) for their full API reference.