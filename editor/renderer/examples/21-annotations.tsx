...
const AnnotationCheckbox = (props: {
	...
	const onClick = React.useCallback(() => {
		updateAnnotationSubscriber.emit(AnnotationUpdateEvent.REMOVE_ANNOTATION_FOCUS);
		updateAnnotationSubscriber.emit(AnnotationUpdateEvent.SET_ANNOTATION_FOCUS, {
			annotationId: id,
		});
	}, [id]);
...
export const useAnnotationsProvider = (
	...
	const annotationInlineCommentProvider = React.useMemo(
		() => ({
			...
			selectionComponent: ExampleSelectionInlineComponent(createNewAnnotationAndReplaceDocument),
			hoverComponent: ExampleHoverInlineComponent(createNewAnnotationAndReplaceDocument),
...
const Annotations = () => {
	...
	const onChange = React.useCallback(
		(evt: React.ChangeEvent<HTMLInputElement>) => {
			...
			const type = checked ? AnnotationMarkStates.ACTIVE : AnnotationMarkStates.RESOLVED;

			const [state, dispatchType]: [AnnotationMarkStates, 'unresolved' | 'resolved'] =
				type === AnnotationMarkStates.ACTIVE
					? [AnnotationMarkStates.ACTIVE, 'unresolved']
					: [AnnotationMarkStates.RESOLVED, 'resolved'];

			updateAnnotationSubscriber.emit(AnnotationUpdateEvent.SET_ANNOTATION_STATE, {
				[id]: {
					id,
					annotationType: AnnotationTypes.INLINE_COMMENT,
					state,
	...
	return (
		<Fragment>
			{Object.entries(state).map(([key, val]) => (
				<AnnotationCheckbox id={key} key={key} state={val} onChange={onChange} />
			))}
		</Fragment>
	);
};
