...
describe('<EmojiPickerList />', () => {
	...
	const renderEmojiPickerList = (customProps: Partial<EmojiPickerListProps> = {}) =>
	...
	describe('list', () => {
		it('should contain search ', async () => {
			...
			expect(wrapper).toHaveCompiledCss('flex-grow', '1');
			expect(wrapper).toHaveCompiledCss('flex-shrink', '1');
			expect(wrapper).toHaveCompiledCss('flex-basis', 'auto');
		});
		...
		it('should show frequently used category first if present', async () => {
			...
			expect(headingItems[0].textContent).toEqual(messages.frequentCategory.defaultMessage);
			expect(headingItems[1].textContent).toEqual(messages.peopleCategory.defaultMessage);
		});
	...
	describe('custom upload display', () => {
		it('should render user custom emojis under Your Uploads', async () => {
			...
			expect(headingItems[1].textContent).toEqual(messages.allUploadsCustomCategory.defaultMessage);

			// expected 3 emojis: foo in "Your Uploads", foo/wtf in "All uploads"
			// With @compiled/react, getting all the imgs is also including the delete button
			// The class for visibility:hidden is present for the component but the style is not being applied
			const unfilteredImages = await screen.findAllByRole('img');
			expect(unfilteredImages).toBeDefined();
			const images = unfilteredImages.filter(
				(image) => image.getAttribute('aria-label') !== 'delete-emoji',
			);
		...
		it('should not render user custom emojis section if user has none', async () => {
			...
			expect(headingItems[0].textContent).toEqual(messages.allUploadsCustomCategory.defaultMessage);
	...
	describe('delete', () => {
		...
		it('should call onEmojiDelete if delete button is clicked', async () => {
			...
			const deleteBtn = await screen.findByTestId(RENDER_EMOJI_DELETE_BUTTON_TESTID);
