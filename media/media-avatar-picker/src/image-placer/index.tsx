...
export class ImagePlacer extends React.Component<ImagePlacerProps, ImagePlacerState> {
	...
	get sourceBounds(): Bounds {
		const { containerWidth, containerHeight } = this.props;
		const { x: originX, y: originY } = this.transformVisibleBoundsToImageCoords(0, 0);
	...
	setZoom(newZoom: number): void {
		...
		const { x: deltaX, y: deltaY } = lastItemBounds.center.sub(imageBounds.center);
	...
	toCanvas = (): HTMLCanvasElement => {
		const { imageElement, sourceBounds, visibleBounds, imageBounds, containerRect, props } = this;
		const { useConstraints, useCircularClipWithActions, backgroundColor } = props;
		const viewInfo = {
			containerRect,
			imageBounds,
			sourceBounds,
