...
export const ArticleBody = (props: Props): React.JSX.Element | null => {
		...
		if (currentIframe !== null && currentIframe.contentWindow !== null) {
           ...
           and of the iframe itself */
				if (iframeContent) {
					...
					currentIframe.contentWindow.document.body.style.height = contentHeight + 'px';
				}
			}
		}
	};
	...
	useEffect(() => {
		...
		const setIframeContent = (body: string = '', onArticleRenderBegin?: () => void) => {
			/**
	...
	}, [props.body, props.bodyFormat, props.onArticleRenderBegin, props.onArticleRenderDone]);
